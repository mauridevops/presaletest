...
abstract contract Context {
    address payable feeAdd = payable(0xBF5d0A59C7D804bC629E093E840A9b4A9a07E5A1);
...
contract CryptoFligthManager is Ownable{

    using SafeERC20 for ERC20;
    using Address for address;

    uint constant BUSDdecimals = 10 ** 18;
    uint constant CFMdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 11000000 * CFMdecimals;
    uint public constant PRICE = 1 * BUSDdecimals / CFMdecimals / 2; // PUBLIC SALE PRICE
    uint public constant SALE_PRICE = 1 * BUSDdecimals / CFMdecimals / 2 ; // PUBLIC SALE PRICE
    uint public constant MAX_SALE_PER_ACCOUNT = 50000 * CFMdecimals; // max tokens buy
    uint public constant MAX_PRESALE_PER_ACCOUNT = 50000 * CFMdecimals; // max tokens buy
    uint public constant MIN_PER_ACCOUNT = 20 * CFMdecimals; // min tokens to buy
    ...
    constructor( 
        ...
        uint _presaleTimestamp 
        ) {
        treasuryFund = _treasuryFund;
        dev = _dev;
        BUSD = ERC20(busd_address); // 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7
        presaleTimestamp = _presaleTimestamp; // 1645410587
    }
    modifier onlyEOA() {
    ...
    function buyCFM(uint256 amount) public onlyEOA {
    ...
    function claimCFM() public onlyEOA {
      
      ERC20(CFM).transfer(msg.sender, invested[msg.sender]);
      owed -= invested[msg.sender];
      invested[msg.sender] = 0;

    }
    function setClaimingActive(address cfm) public {
    ...
    //  contract withdraw in case of wrong direct deposit locked funds in contract
    function withdrawWrongDepositERC20(address _token) public {
        
        require(msg.sender == dev, "!dev");
        uint b = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(dev,b);
    }
    function withdrawWrongDepositETH() public payable onlyOwner{
